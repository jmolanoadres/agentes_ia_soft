...
class MetricsCollector:
    ...
    def record_agent_metric(self, agent_id: str, metric_name: str, value: float) -> None:
        ...
        metric = AgentMetric(
            agent_id=agent_id, metric_name=metric_name, value=value, timestamp=datetime.now()
        )
    ...
    def record_system_metric(self, metric_name: str, value: float, unit: str = "") -> None:
        """Registrar una métrica del sistema."""
        metric = SystemMetric(
            metric_name=metric_name, value=value, unit=unit, timestamp=datetime.now()
        )
    ...
    def calculate_kpis(self) -> dict[str, Any]:
        ...
        for agent_id in self._agent_metrics.keys():
            ...
            total_tasks = sum(
                1 for m in metrics if m.metric_name in ["tasks_completed", "tasks_failed"]
            )
            utilization[agent_id] = total_tasks
        kpis["agent_utilization"] = utilization
