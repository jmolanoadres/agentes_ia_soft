...
class MaintenanceAgent(BaseAgent):
    ...
    def _register_capabilities(self) -> None:
        ...
        self.add_capability(
            Capability(
                name="monitor_health", description="Monitorear salud del sistema", version="1.0.0"
            )
        )
        self.add_capability(
            Capability(
                name="detect_issues", description="Detectar issues y alertas", version="1.0.0"
            )
        )
        self.add_capability(
            Capability(
                name="analyze_logs", description="Analizar logs del sistema", version="1.0.0"
            )
        )
        self.add_capability(
            Capability(
                name="generate_reports", description="Generar informes de salud", version="1.0.0"
            )
        )
        self.add_capability(
            Capability(
    ...
    async def _analyze_logs(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        analysis = {
            ...
            "recommendations": ["Revisar timeouts de conexión", "Optimizar validación de entrada"],
        }
    ...
    async def _suggest_improvements(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"suggestions": suggestions, "count": len(suggestions), "prioritized": True}
