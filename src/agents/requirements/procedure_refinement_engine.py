...
class ProcedureRefinementEngine:
    ...
    def __init__(
        self, config: dict[str, Any] | None = None, decision_engine: Any = None
    ) -> None:
        ...
        self._safe_auto_apply: list[str] = list(
            self._config.get(
                "safe_auto_apply",
    ...
    # 2)
    def record_feedback(self, skill_name: str, score: float, details: str | None = None) -> None:
    ...
    # 3)
    def analyze_performance(self, skill_name: str | None = None) -> dict[str, Any]:
    ...
    # 7)
    def learn_ambiguity_pattern(self, term: str, context: str | None = None) -> None:
    ...
    def _calculate_skill_health(self, skill: SkillRecord) -> float:
        ...
        feedback = (
            skill.last_feedback_score if skill.last_feedback_score > 0 else skill.success_rate
        )
        feedback_component = feedback * 0.30
        return float(max(0.0, min(1.0, success_component + time_component + feedback_component)))
    ...
    # 18)
    def _make_action(
        self, skill_name: str, action_type: str, description: str, impact_score: float
    ) -> RefinementAction:
