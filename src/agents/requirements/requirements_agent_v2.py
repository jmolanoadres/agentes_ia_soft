...
class RequirementsAgentV2(BaseAgent):
    ...
    def __init__(self) -> None:
        ...
        self._config: RequirementsConfig = get_config()
        ...
        self._current_srs: SoftwareRequirementsSpec | None = None
    ...
    async def execute(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            return TaskResult(
                task_id=task.id, status=TaskStatus.FAILED, error=str(exc), execution_time=elapsed
            )
    ...
    async def _run_full_pipeline(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        validation = await self._validate_requirements(
            {"requirements": [self._req_to_dict(r) for r in requirements]}
        )
        conflicts = await self._detect_conflicts(
            {"requirements": [self._req_to_dict(r) for r in requirements]}
        )
        prioritization = await self._prioritize_requirements(
            {"requirements": [self._req_to_dict(r) for r in requirements]}
        )
        ...
        use_cases = await self._create_use_cases(
            {"requirements": [self._req_to_dict(r) for r in requirements]}
        )
    ...
    async def _detect_conflicts(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        for r in requirements:
            ...
            if r.id in r.dependencies:
                issues.append(
                    {"type": "self_dependency", "requirement_id": r.id, "severity": "critical"}
                )
    ...
    async def _learn_from_feedback(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if task_type:
            skill = self._skill_stats.setdefault(
                task_type,
                {
                    "exec": 0,
                    "ok": 0,
    ...
    async def _get_agent_metrics(self, input_data: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "memory_stats": self._requirements_memory.stats()
            if self._requirements_memory
            else None,
        }
    ...
    async def _search_similar_requirements(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        results = self._requirements_memory.search_similar(
            query, top_k=int(input_data.get("top_k", 5))
        )
        return {
            "query": query,
            "results": results,
    ...
    def _collect_assumptions(
        self, input_data: dict[str, Any], requirements: list[Requirement]
    ) -> list[str]:
        ...
        for r in requirements:
            ...
            if any(k in desc for k in ("segundo", "latencia", "p95", "rendimiento", "500ms")):
                assumptions.append(
                    "Se dispone de métricas/criterios de rendimiento cuantificables (p.ej. p95) para aceptación."
                )
                break
    ...
    async def _validate_assumptions(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        payload["next_step"] = (
            "Responde con los números que NO te gustan (o 0 si todos están bien)."
        )
        return payload
    ...
    async def _analyze_requirements(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        for r in requirements:
            ...
            if not r.description:
                issues.append(
                    {"type": "missing_description", "requirement_id": r.id, "severity": "high"}
                )
            ...
            if r.id in r.dependencies:
                issues.append(
                    {"type": "circular_dependency", "requirement_id": r.id, "severity": "critical"}
                )
    ...
    async def _validate_requirements(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        for r in requirements:
            ...
            results.append(
                {"requirement_id": r.id, "valid": len(problems) == 0, "issues": problems}
            )
    ...
    def get_current_srs(self) -> SoftwareRequirementsSpec | None:
