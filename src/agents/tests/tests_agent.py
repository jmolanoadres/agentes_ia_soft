...
class TestsAgent(BaseAgent):
    ...
    def _register_capabilities(self) -> None:
        ...
        self.add_capability(Capability(
            name="generate_tests",
            description="Generar casos de prueba",
            version="1.0.0"
        ))
        self.add_capability(Capability(
            name="execute_tests",
        ...
        self.add_capability(Capability(
            ...
            description="Analizar cobertura de código",
            version="1.0.0"
        ))
        self.add_capability(Capability(
            name="generate_reports",
            description="Generar informes de pruebas",
            version="1.0.0"
        ))
        self.add_capability(Capability(
...
'''
    ...
    async def _generate_reports(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if test_results.get("failed", 0) > 0:
            report["recommendations"].append("Revisar casos de prueba fallidos")
        
        if test_results.get("skipped", 0) > 5:
            report["recommendations"].append("Reducir tests saltados")
