...
@pytest.fixture
def deploy_agent() -> Any:
    """Create deploy agent instance."""
    return DeployAgent()


@pytest.mark.asyncio
async def test_initialization(deploy_agent: Any) -> None:
...
@pytest.mark.asyncio
async def test_create_pipeline(deploy_agent: Any) -> None:
    ...
    task = Task(
        ...
        input_data={"project_name": "Test Project", "stages": ["build", "test", "deploy"]},
    )
...
@pytest.mark.asyncio
async def test_configure_deployment(deploy_agent: Any) -> None:
    ...
    task = Task(
        ...
        input_data={"environment": "production", "image_tag": "v1.0.0", "replicas": 3},
    )
...
@pytest.mark.asyncio
async def test_execute_deployment(deploy_agent: Any) -> None:
    ...
    task = Task(
        ...
        input_data={"config_id": "config-1", "environment": "staging"},
    )
...
@pytest.mark.asyncio
async def test_rollback(deploy_agent: Any) -> None:
    """Test rollback."""
    await deploy_agent.initialize()
    ...
    task = Task(
        id="test-4",
        type="rollback",
        input_data={"deployment_id": "deploy-1", "target_version": "v0.9.0"},
    )
...
@pytest.mark.asyncio
async def test_shutdown(deploy_agent: Any) -> None:
    """Test agent shutdown."""
    await deploy_agent.initialize()
