...
@pytest.mark.asyncio
async def test_initialization(design_agent):
...
@pytest.mark.asyncio
async def test_create_architecture(design_agent):
    ...
    task = Task(
        ...
        input_data={
            ...
            "requirements": [
                {"id": "req-1", "type": "functional", "title": "Feature 1"}
            ]
        }
    )
...
@pytest.mark.asyncio
async def test_select_patterns(design_agent):
...
@pytest.mark.asyncio
async def test_validate_design(design_agent):
    ...
    task = Task(
        ...
        input_data={
            "architecture": {
                "components": [
                    {"name": "Business Layer", "description": "Layer"}
                ]
            },
            "requirements": [
...
@pytest.mark.asyncio
async def test_shutdown(design_agent):
    """Test agent shutdown."""
    await design_agent.initialize()
